...
public class RandomWayCoordinates {
     ...
     * 
     * @param rows
     *            rowcount of the labyrinth field
     * @param cols
     *            colcount of the labyrinth field
     * @exception IllegalStateException
     *                thrown when trying to set a wrong value of rows and cols
     */
    ...
    public boolean findRandomWayCoordinates(LabyrinthField labyrinth_field) {
        ...
        for (rows_count = 0; rows_count < rows_; rows_count++) {
            for (cols_count = 0; cols_count < cols_; cols_count++) {
                if ((labyrinth_field.isWay(rows_count, cols_count) == true)
                        && (labyrinth_field
                                .isStartpoint(rows_count, cols_count) == false))
                    way_cell_counter++;
     ...
     * @return random_row
     * @exception IllegalStateException
     *                thrown when rows is not set
     */
    public int getRandomWayRowCoordinate() throws IllegalArgumentException {
        if (random_row_ < 1)
            throw (new IllegalArgumentException("random_row_ must be > 1"));
        return random_row_;
    }
     ...
     * @return random_col
     * @exception IllegalStateException
     *                thrown when col is not set
     */
    public int getRandomWayColCoordinate() throws IllegalArgumentException {
        if (random_col_ < 1)
            throw (new IllegalArgumentException("random_col_ must be > 1"));
        return random_col_;
    }
