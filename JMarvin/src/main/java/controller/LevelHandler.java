...
public class LevelHandler {
    ...
    public void loadGameXmlFile(String filename) {
        try {
             ...
             * true); if (xmlReader.getFeature(id)) {
             * System.out.println("Parser is validating."); } else {
             * System.out.println("Parser is not validating."); } } catch
             * (SAXNotRecognizedException e) {
             * System.out.println("Can't tell."); } catch
            ...
            xmlReader.parse(new InputSource(LevelHandler.class.getResourceAsStream("/data/config/games/Game.xml")));
            ...
            if (is_error_ == false) {
                ...
                diamond_picture_ = game_xml_content_handler_
                        .getDiamondPicture();
    ...
    /**
     * loadLevel is used to fill load_labyrinth_ with a LoadLabyrinth object
     * which can be either a new one or a copy of the last one.
     *
     * @param number
     *            The level choosed in the LabyrinthChoosePanel
     * @author user
     */
    public void loadLevel(int number) {
        if (is_error_ == false) {
            file_ = game_xml_content_handler_.getFile(number); // get filename
                                                               // from Game.xml
            if (file_ != null) {
                is_not_default_style_ = false;
    ...
    public void loadLevelFromString(String input) {
        if (is_error_ == false) {
            ...
            load_labyrinth_ = new LoadLabyrinth(input_, document_base_,
                    mark_array_, true);
            is_not_default_style_ = load_labyrinth_.isStyle();
        }
    }
