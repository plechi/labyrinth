...
public class PrintLabyrinth extends JPanel {
    ...
    private Image img_cellmark0_, img_cellmark1_, img_cellmark2_,
            img_cellmark3_, img_cellmark_;
    ...
    public PrintLabyrinth(HashMap brick_images, IntStringMap mark_array,
    ...
    public void paintComponent(Graphics g) {
        ...
        if ((labyrinth_field_ != null) && (robot_ != null)) {
            while (rows_counter < labyrinth_field_.getNumRows()) {
                while (cols_counter < labyrinth_field_.getNumCols()) {
                    if (labyrinth_field_.isWay(rows_counter, cols_counter) == true) {
                        ...
                        if (labyrinth_field_.containsDiamond(rows_counter,
                                cols_counter) == true)
                            g.drawImage(img_diamond_, hposition, vposition,
                                    this);

                        if (labyrinth_field_.getCellMark(rows_counter,
                                cols_counter) == 0)
                            g.drawImage(img_cellmark0_, hposition, vposition,
                                    this);

                        if (labyrinth_field_.getCellMark(rows_counter,
                                cols_counter) == 1)
                            g.drawImage(img_cellmark1_, hposition, vposition,
                                    this);

                        if (labyrinth_field_.getCellMark(rows_counter,
                                cols_counter) == 2)
                            g.drawImage(img_cellmark2_, hposition, vposition,
                                    this);

                        if (labyrinth_field_.getCellMark(rows_counter,
                                cols_counter) == 3)
                            g.drawImage(img_cellmark3_, hposition, vposition,
                                    this);

                        if (labyrinth_field_.getCellMark(rows_counter,
                                cols_counter) > 3)
                            g.drawImage(img_cellmark_, hposition, vposition,
                                    this);
            ...
            img_robot_ = (Image) brick_images_.get("LAB_ROBOT_"
                    + condition_color + "_" + direction_string);
