...
public class RuleTreePanel extends javax.swing.JPanel {
    ...
    public RuleTreePanel(HashMap brick_images, IntStringMap mark_array,
    ...
    public RuleTreePanel(HashMap brick_images, IntStringMap mark_array,
            IntStringMap state_array, Dimension dimension) {
        this.setLayout(null);
        this.setBackground(Color.ORANGE);
        brick_images_ = brick_images;
        mark_array_ = mark_array;
    ...
    public void drawAllRules() {
        ...
        for (int index = 0; index < rule_list_.size(); index++) {
            ...
            this.add(tmp);
            tmp.setBounds(0,
                    (rule_list_.size() - index - 1) * CONDITION_HEIGHT,
    ...
    public void addEmptyRule(int id) {
        RulePanel rule = new RulePanel(brick_images_, mark_array_,
                state_array_, id, this);
        rule_list_.add(rule);
        instruction_list_.addInstruction(new Instruction(id));
        // this.add(rule);
        // rule.setBounds(0, rule_list_.size() * CONDITION_HEIGHT,
        // CONDITION_WIDTH, CONDITION_HEIGHT);
        // adjustSize();
    ...
    public void setInstructionList(InstructionList instruction_list) {
        ...
        for (int index = 0; index < instruction_list_.size(); index++) {
            ...
            RulePanel rule = new RulePanel(brick_images_, mark_array_,
                    state_array_, dummy, dummy.getId(), this);
            rule_list_.add(rule);
        }
        drawAllRules();
    }
    ...
    private void adjustSize() {
        if ((rule_list_.size()) * CONDITION_HEIGHT > start_dimension_
                .getHeight()) {
            this.setSize(new Dimension(CONDITION_WIDTH, (rule_list_.size())
                    * CONDITION_HEIGHT));
            this.setPreferredSize(new Dimension(CONDITION_WIDTH, (rule_list_
                    .size()) * CONDITION_HEIGHT));
        } else {
            this.setSize(start_dimension_);
