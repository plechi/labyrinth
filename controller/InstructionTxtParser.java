 ...
 * parses the config xml file to get the settings
 * @param url
 */
...
//-----------------------------------------------------------------------------
 /**
 * checks the condtion string for syntax errors and generates a new condition with 
 ...
 */
  ...
  else if(condition_sensors.regionMatches(false,0,way_string_,0,way_string_.length())==true)
    condition_sensors=condition_sensors.substring(way_string_.length());
  else if(condition_sensors.length()==0)
    throw(new IllegalArgumentException("Condition Syntax Error in line "+linecount+": wrong parameter count for sensors"));
  else
    throw(new IllegalArgumentException("Condition Syntax Error in line "+linecount+": wrong parameter for left sensor"));
  ...
  //       System.out.println("wall_ahead");
    ...
    throw(new IllegalArgumentException("Condition Syntax Error in line "+linecount+": wrong parameter for ahead sensor"));
  ...
  condition_sensors=condition_sensors.trim();
   if(condition_sensors.regionMatches(false,0,wall_string_,0,wall_string_.length())==true)
   {
     ...
     throw(new IllegalArgumentException("Condition Syntax Error in line "+linecount+": wrong parameter for right sensor"));

   condition_sensors=condition_sensors.trim();
   if(condition_sensors.length()!=0)
     throw(new IllegalArgumentException("Condition Syntax Error in line "+linecount+": wrong parameter count for sensors"));

  return condi = new Condition(state_array_.setString(condition_state),mark_array_.setString(condition_cellmark),wall_left,wall_right,wall_ahead);
  }
  
//-----------------------------------------------------------------------------
 /**
 * checks the action string for syntax errors and generates a new action with 
 * the string informations
 * @param actionstring
 * @param linecount
 * @return action
 ...
 */
    ...
    if((comment_out!=-1)&&((comment_in!=-1))&&(comment_out < comment_in))
    {
      String tmp_before=line.substring(0, comment_out);
      String tmp_post=line.substring(comment_in+comment_out_end_string_.length());
    ...
    {
      line=line.substring(comment_in+(comment_out_end_string_.length()));
    ...
    if(line.startsWith(comment_out_line_string_)||line.length()==0)
     ...
     else
     {
	  ...
	  if (line.startsWith(deactivate_instruction_string_))
	  {
		  ...
		  if(deaktivate==-1)
        		throw(new IllegalArgumentException("Syntax Error in line "+linecount));
          else
          {
	          line=line.substring(deaktivate+deactivate_instruction_string_.length(), line.length());
	          line=line.trim();
	      }
  	  }
	  else
