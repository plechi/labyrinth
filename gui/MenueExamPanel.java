...
{	
	...
	timer_.schedule(new TimerTask(){public void run()
	{
	    ...
	    //SqlCommunicator.add_log("Next button pressed");      
	      ...
	      {
	        current_selected_=(Integer)levels_.remove(0);
        	simulation_control_.drawNewLabyrinthSetTimer(current_selected_);
        	SqlCommunicator.add_log("Weiter Knopf getrückt", 16);
    	  }
    	  else
    	  {
			this_.remove(next_button_);
			repaint();
			SqlCommunicator.add_log("Alle Levels bearbeitet!", 16);
