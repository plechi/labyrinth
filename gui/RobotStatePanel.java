...
public class RobotStatePanel extends JPanel 
{
    ...
    public RobotStatePanel(HashMap brick_images, IntStringMap state_array, boolean on_cond_panel, UpdateObject parent_panel) 
    {
        super();
     ...
     public RobotStatePanel(HashMap brick_images, IntStringMap state_array, boolean on_cond_panel, UpdateObject parent_panel, String name) 
     {
        this(brick_images, state_array, on_cond_panel, parent_panel);
        name_=name;
 	 }  
      
     public RobotStatePanel(HashMap brick_images, int init_state, IntStringMap state_array, boolean on_cond_panel, UpdateObject parent_panel) 
     {
        this(brick_images, state_array, on_cond_panel, parent_panel);
        setInternalState(init_state);
 	 }
 	 
 	 public RobotStatePanel(HashMap brick_images, int init_state, IntStringMap state_array, boolean on_cond_panel, UpdateObject parent_panel, String name) 
     {
    ...
    private void paintState() 
    {
        ...
        case BLUE:
            current_image_ = (Image) brick_images_.get("COND_BLUE" + postnote);
						this.setToolTipText("Status: "+state_array_.getFromInt(internal_state_-1));
            break;
        case GREEN:
       ...
       case RED:
            ...
            break;
       case YELLOW:
            current_image_ = (Image) brick_images_.get("COND_YELLOW" + postnote);
						this.setToolTipText("Status: "+state_array_.getFromInt(internal_state_-1));
						break;
				default:
	...
	private void initMouseListener() 
	{
        this.addMouseListener(new java.awt.event.MouseAdapter() 
        {
            public void mouseClicked(java.awt.event.MouseEvent evt) 
            {
	            if(name_!=null)
	            {
		            SqlCommunicator.add_log(name_+" geändert zu "+state_array_.getFromInt(internal_state_-1), 512);
		        }
		        else
		        {
			        SqlCommunicator.add_log("Zustand geändert zu "+state_array_.getFromInt(internal_state_-1), 512);
			    }
                internal_state_ = ((internal_state_ + 1) % STATE_COUNT);
                /*String placing = "Condition";
                if (placing_ == PLACING_ACTION)
                	placing = "Action";
                SqlCommunicator.add_log("Regel " + Integer.toString(id_) + ": " + placing + " Zustand geändert zu " + labyrinth_.state_array_.getFromInt(internal_state_-1));
